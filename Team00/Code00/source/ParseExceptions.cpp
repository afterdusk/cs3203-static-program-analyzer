...
std::string ParseException::toString(std::vector<Token> tokens) {
  std::string string;
...
InvalidExpressionException::InvalidExpressionException(
    int line, std::vector<Token> content)
...
InvalidConditionException::InvalidConditionException(int line,
                                                     std::vector<Token> content)
    : line_no(line), line_content(ParseException::toString(content)) {}
const char *InvalidConditionException::what() const throw() {
  std::stringstream ss;
  ss << "Error: Line " << line_no << ": The condition expression on the line \""
     << line_content
     << "\" is invalid. Please check syntax grammar for SIMPLE program.";
  std::string str = ss.str();
  char *copy = new char[str.size() + 1];
